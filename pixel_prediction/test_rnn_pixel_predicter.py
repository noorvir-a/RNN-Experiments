...
MODEL_NUM = 2               # Choose which model to run options (0,1,2,3) for (32,64,128,s32)
PRED_SIZE = 3              # Choose the number of pixels to predict (0,1,2,3) for (1,10,28,300)
# ==================================================================================================

np.random.seed(140693)

class RNN:
    def __init__(self, batch_size=500, num_hidden=128, num_linear=100, num_layers=1):
    ...
    def run(self, model_filename, orgb_images, ppds_images, test_type='single'):

        # List of predicted images
        pred_images = np.array([])
        ...
        with tf.Session() as sess:
            ...
            # Variables to store the 100 predicted images (pixels) and the associated
            # probabilities over 10 samples
            pred_images = np.zeros((100, 10, 784))
            pred_ns = np.zeros((100, 10, 784))


            if test_type == 'single':
                # pixel_prob = sess.run(y_pred, feed_dict={self.x: orgb_images})
                # pixel_pred = np.random.binomial(1, pixel_prob)
                # image = np.append(pixel_pred, [0])
                pass


            elif test_type == 'sequence':

                images = np.reshape(ppds_images, (100, 784, 1))
                images = np.repeat(images, 10, axis=0)

                # Used for printing the test loss before the variable is updated
                count = 0

                for px in range(484, 783):
                    print('Pixel number: ', px)
                    pixel_ns, pixel_prob = sess.run([y_ns, y_pred], feed_dict={self.x: images})
                    # Pad a zeros at the front to match the indices of the predicted image
                    # to the original
                    pixel_prob = np.concatenate((np.zeros((1000, 1)), pixel_prob), axis=1)
                    pixel_pred = np.random.binomial(1, pixel_prob)

                    images = np.reshape(images, (1000, 784))
                    images = np.concatenate((images[:, :px], pixel_pred[:, px:px+1],
                                            images[:, px+1:]), axis=1)
                    images = np.reshape(images, (1000, 784, 1))

                    if count == 0:
                        # Testing loss over entire image wrt. ground truth
                        pixel_ns = np.concatenate((np.zeros((1000, 1)), pixel_ns), axis=1)
                        test_images = np.copy(images)
                        test_images = np.reshape(test_images, (1000, 784))
                        test_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(pixel_ns,
                                                                                       test_images),
                                                                                       axis=1)
                ...
                pred_images = np.reshape(images, [100, 10, 784])
                pred_ns = np.reshape(pixel_ns, [100, 10, 784])
                # pred_images28 = np.reshape(image[0], [28, 28])
    ...
    def metrics(self, filename, pred_size):
        ...
        ip_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(pred_ns, pred_images), 1)
    ...
    def save_bin_images(self, filename, loss_filename, pred_size):
        ...
        for image_id in image_idx:
            ...
            img_filename = FAIL_FOLDER_NAME + 'I' + str(image_id+1) + 'FAIL_sm_' + str(sm_max+1) + \
                           '.png'
            plt.imsave(img_filename, np.array(imgs[image_id, sm_max, :]).reshape(28, 28))

            # Save good example
            img_filename = GOOD_FOLDER_NAME + 'I' + str(image_id+1) + 'GOOD_sm_' + str(sm_min+1) + \
                            '.png'
            plt.imsave(img_filename, np.array(imgs[image_id, sm_min, :]).reshape(28, 28))

            # Save high variance example
            img_filename = VAR_FOLDER_NAME + 'I' + str(image_id+1) + 'VAR_sm' + str(sm_mid+1) + \
                            '.png'
...
def main():
    ...
    orgb_images, ppds_images = extract_data.sample_images()
    ...
    if RUN_TEST:
        ...
        rnn_obj.run(model_filename, orgb_images, ppds_images, test_type='sequence')
