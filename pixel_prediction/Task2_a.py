...
isTRAIN = False             # Set True to train
EXTRACT_MNIST = False       # Set True if the pickle mnist file doesn't already exist
# ==================================================================================================


class RNN:
    def __init__(self, batch_size=500, learning_rate=0.0005, num_epochs=100, num_hidden=128,
                 num_linear=100):
    ...
    def model(self):
        ...
        gru = rnn_cell.DropoutWrapper(gru, output_keep_prob=0.6)
    ...
    def run(self, data, is_train):
        ...
        ts = datetime.datetime.fromtimestamp(t).strftime(
            '%Y%m%d %H-%M-%S')
        FILENAME = 'T2_' + str(self.num_hid_units) + '_' + ts
        ...
        acc_train = np.zeros((self.num_epochs * int(
                                            NUM_TRAIN_SAMPLES/self.batch_size), 1))
        ...
        if is_train:
            ...
            with tf.Session() as sess:
                ...
                # Output CSV
                csv_filename = LOG_FOLDER + 'T2_' + str(
                    ...
                    for epoch in range(self.num_epochs):
                        ...
                        for batch in range(int(NUM_TRAIN_SAMPLES / self.batch_size)):
                            ...
                            if (it + 1) % 100 == 0:
                                ...
                                csv_writer.writerow([epoch, batch+1, hr,
                                                     m, int(sec),
                                                     str(acc_train[it]),
                                                     str(loss)])
