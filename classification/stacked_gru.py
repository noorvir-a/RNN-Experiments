...
# Date: 24-02-2017
# Stacked GRU: 3 recurrent layers with 32 units each
#
...
LOAD_MODEL_FILENAME = "./T1_d.ckpt"
# ==================================================================================================

class RNN:
    def __init__(self, batch_size=500, learning_rate=0.0006, num_epochs=1, num_hid_units=32,
        ...
        # Weights and biases
        self.weights_H2L = tf.Variable(tf.random_normal([self.num_hid_units, self.num_lin_units]))
        self.weights_L2O = tf.Variable(tf.random_normal([self.num_lin_units, NUM_CLASSES]))
    ...
    def run(self, data, is_train):
        ...
        ts = datetime.datetime.fromtimestamp(t).strftime(
            '%Y%m%d%H%M%S')
        FILENAME = 'T1_d_' + str(self.num_hid_units) + '_' + ts
        ...
        if is_train:
            trainer = tf.train.AdamOptimizer(
                                        self.learning_rate).minimize(ce_loss)
            ...
            with tf.Session() as sess:
                ...
                # Output CSV
                csv_filename = LOG_FOLDER + 'T1_d_' + str(
                    self.num_hid_units) + '_' + ts + '.csv'
                    ...
                    for epoch in range(self.num_epochs):
                        ...
                        for batch in range(int(NUM_TRAIN_SAMPLES / self.batch_size)):
                            ...
                            if (it + 1) % 100 == 0:
                                ...
                                btest_y = data['test_y'][:]
                                acc_train[it] = sess.run(accuracy, feed_dict={self.x: btest_x,
                                                                              self.y: btest_y})
